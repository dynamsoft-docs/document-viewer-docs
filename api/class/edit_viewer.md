...
| [`getCropRect()`](#getcroprect)         | Get the crop rectangle selection.                            |
| [`setCropRect()`](#setcroprect)         | Set a crop rectangle selection on the current page. *This method is only available when [`toolMode`](#toolmode) is `crop` mode.* |
...
| [`cropRectDrawn`](#croprectdrawn)       | Triggered when a rectangle selection is drawn.               |
| [`cropRectDeleted`](#croprectdeleted)     | Triggered when the rectangle selection is deleted.           |
| [`cropRectModified`](#croprectmodified)    | Triggered when the crop rectangle selection is modified.     |
...
```typescript
const viewerUi = DDV.getDefaultUiConfig("eidtViewer");
...
Get the crop rectangle selection.
...
- If there is no crop rectangle selection, returns `null`.

### setCropRect()

> *This method is only available when [`toolMode`](#toolmode) is `crop` mode.*

Set a crop rectangle selection on the current page.
