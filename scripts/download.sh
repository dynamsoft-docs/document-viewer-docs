...
# Define variables
DOCS_REPO_URL="https://github.com/dynamsoft-docs/document-viewer-docs/archive/refs/heads/preview.zip"
...
FINAL_DIR="document-viewer-docs-preview"
...
# Download first repository (document-viewer-docs)
echo "Downloading document-viewer-docs repository..."
curl -L -o docs.zip "$DOCS_REPO_URL"

# Extract first repository
echo "Extracting document-viewer-docs..."
unzip -q docs.zip

# Get the extracted directory name (dynamic detection)
DOCS_EXTRACTED_DIR=$(find . -maxdepth 1 -type d -name "document-viewer-docs-*" | head -1 | sed 's|^\./||')
